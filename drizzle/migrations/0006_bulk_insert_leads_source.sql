...
BEGIN
  ...
  WITH src AS (
    SELECT NULLIF(TRIM(elem->>'name'),'') AS name,
           ...
           NULLIF(TRIM(elem->>'source'),'') AS source,
           CASE
  ...
  ins AS (
    INSERT INTO public.leads (name, phone_number, email, city, source, lead_received_date, status_id)
    SELECT public.title_case(s.name), s.phone, s.email, public.title_case(s.city), s.source, s.received_date, new_status
    FROM valid s
    WHERE NOT EXISTS (
