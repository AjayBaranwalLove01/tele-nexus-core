...
BEGIN
  ...
  WHERE h.activity_date BETWEEN _from AND _to
    AND h.activity_type IN ('call_logged','remark_added','status_changed','followup_set','completed','reopened')
    AND h.activity_source <> 'migration'
...
BEGIN
  ...
  WITH h AS (
    ...
    WHERE x.activity_date BETWEEN _from AND _to
      AND x.activity_type IN ('call_logged','remark_added','status_changed','followup_set','completed','reopened')
      AND x.activity_source <> 'migration'
